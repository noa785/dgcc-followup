...
with st.sidebar:
    ...
    st.caption("Backups are created automatically in ./backups and an Excel snapshot is saved as FollowUp_AutoBackup.xlsx")

# First run: ensure DB & tables exist
init_db()

tabs = st.tabs(["Tasks", "Change Log", "SLA Policies", "Owners", "Dashboard & Export", "Import Excel (optional)"])
...
with tabs[0]:
    ...
    with st.form("task_form", clear_on_submit=True):
        ...
        Category = c11.selectbox("Category", ["", "Data Cleaning", "File Merge/ETL", "Scheduling", "Reporting", "Dashboard",
        ...
        if submitted:
            if not Task or not DueDate:
                ...
                row = dict(Unit=Unit, Role=Role, Task=Task, Week=int(Week), Status=Status or None,
    ...
    if df.empty:
        ...
        edited_id = st.number_input("Edit Task ID", min_value=0, value=0, step=1, help="Enter the 'id' to edit; see table below.")
        if edited_id:
            ...
            if not row.empty:
                ...
                with st.form("edit_form"):
                    new_status = st.selectbox("Status", ["Not Done","Under Progress","Rescheduled","Blocked","Done"], index= ["Not Done","Under Progress","Rescheduled","Blocked","Done"].index(r.get("Status") or "Not Done"))
                    ...
                    if save:
                        upsert_task({"Status": new_status, "DueDate": str(new_due), "Owner": new_owner, "Notes": new_notes}, task_id=edited_id)
                        auto_backup_now()
    ...
    with st.form("cr_form", clear_on_submit=True):
        ...
        if submit:
            if not Change_ID:
                ...
                conn = get_conn()
                conn.execute("""INSERT INTO change_log(Change_ID,Date,Requested_By,Description,Impact,Approved_By,Status,Linked_Task)
                                VALUES(?,?,?,?,?,?,?,?)
                             ...
                             """, (Change_ID, str(Date), Requested_By, Description, Impact, Approved_By, Status, int(Linked_Task) if Linked_Task else None))
                conn.commit()
                conn.close()
                auto_backup_now()
                st.success("✅ Saved & backup updated.")
    ...
    with st.form("sla_form", clear_on_submit=True):
        ...
        if submit:
            if not Category or not Priority:
                ...
                conn = get_conn()
                conn.execute("INSERT INTO sla_policies(Category,Priority,TargetDays,Notes) VALUES(?,?,?,?)", (Category, Priority, int(TargetDays), Notes))
                conn.commit()
                conn.close()
                auto_backup_now()
    ...
    with st.form("owners_form", clear_on_submit=True):
        ...
        if submit:
            if not Owner:
                ...
                conn = get_conn()
                conn.execute("INSERT INTO owners(Owner,Email,Role,Unit) VALUES(?,?,?,?)", (Owner, Email, Role, Unit))
                conn.commit()
                conn.close()
                auto_backup_now()
...
# --- Import Excel Tab ---
with tabs[5]:
    st.subheader("Import tasks from Excel (optional, one-time or repeated)")
